...
class MetaModel(api.Meta):
    ...
    def __init__(self, name, bases, attrs):
        ...
        if not hasattr(self, '_module'):
            assert self.__module__.startswith('etwork.addons.'), \
                "Invalid import of %s.%s, it should start with 'etwork.addons'." % (self.__module__, name)
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    the :attr:`~etwork.models.BaseModel._register` attribute may be set to False.
    """
    __slots__ = ['env', '_ids', '_prefetch_ids']

    _auto = False
    """Whether a database table should be created.
    If set to ``False``, override :meth:`~etwork.models.BaseModel.init`
    ...
    .. tip:: To create a model without any table, inherit
            from :class:`~etwork.models.AbstractModel`.
    """
    ...
    .. warning::
      ...
      :attr:`~etwork.models.Model._inherits`-ed models, the inherited field will
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = etwork.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or etwork.fields.Datetime.now()
    ...
    def load(self, fields, data):
        ...
        for field_path in fields:
            ...
            if isinstance(model_fields[field_path[0]], etwork.fields.Many2one):
            ...
            for field_name in field_path:
                ...
                if isinstance(model_fields[field_name], etwork.fields.One2many):
    ...
    def _add_fake_fields(self, fields):
        from etwork.fields import Char, Integer
...
%(fields_list)s""") % {
    ...
    def _check_company(self, fnames=None):
        ...
        :raises UserError: if the `company_id` of the value of any field is not
            ...
            :class:`~etwork.addons.base.models.res_company`).

        For :class:`~etwork.addons.base.models.res_users` relational fields,
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~etwork.fields.Integer`,
          :class:`~etwork.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~etwork.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~etwork.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~etwork.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~etwork.fields.Date` and
              :class:`~etwork.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`etwork.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`etwork.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _openerp/models/relationals/format:

          :class:`~etwork.fields.One2many` and
          :class:`~etwork.fields.Many2many` use a special "commands" format to
    ...
    def _compute_field_value(self, field):
        etwork.fields.determine(field.compute, self)
    ...
    def ensure_one(self):
        ...
        :raise etwork.exceptions.ValueError: ``len(self) != 1``
        """
        try:
    ...
    def with_env(self, env):
        ...
        :param env:
        :type env: :class:`~etwork.api.Environment`
    ...
    def with_company(self, company):
        ...
        :type company: :class:`~etwork.addons.base.models.res_company` or int
        ...
        for field, value in sorted(field_values, key=is_monetary):
            ...
            if field.relational:
                ...
                for invf in self._field_inverses[field]:
                    ...
                    # /home/dle/src/etwork/master-nochange-fp/etwork/addons/test_new_api/tests/test_new_fields.py
    ...
    def _populate_factories(self):
        ...
        See :mod:`etwork.tools.populate` for population tools and applications.
