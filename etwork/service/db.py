...
import etwork
from etwork import SUPERUSER_ID
from etwork.exceptions import AccessDenied
import etwork.release
import etwork.sql_db
import etwork.tools
from etwork.sql_db import db_connect
from etwork.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not etwork.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and etwork.tools.config.verify_admin_password(passwd):
        return True
    raise etwork.exceptions.AccessDenied()

# This should be moved to etwork.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = etwork.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            etwork.modules.db.initialize(cr)
            etwork.tools.config['load_language'] = lang
            cr.commit()

        registry = etwork.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = etwork.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = etwork.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = etwork.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = etwork.tools.config['db_template']
    ...
    if etwork.tools.config['unaccent']:
        try:
            db = etwork.sql_db.db_connect(name)
            with closing(db.cursor()) as cr:
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    etwork.sql_db.close_db(db_original_name)
    db = etwork.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = etwork.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = etwork.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = etwork.tools.config.filestore(db_original_name)
    to_fs = etwork.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    etwork.modules.registry.Registry.delete(db_name)
    etwork.sql_db.close_db(db_name)
    ...
    fs = etwork.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'etwork_dump': '1',
        'db_name': cr.dbname,
        'version': etwork.release.version,
        'version_info': etwork.release.version_info,
        'major_version': etwork.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = etwork.tools.config.filestore(db_name)
            ...
            etwork.tools.exec_pg_command(*cmd)
            if stream:
                etwork.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                etwork.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = etwork.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        if etwork.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = etwork.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = etwork.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    etwork.modules.registry.Registry.delete(old_name)
    etwork.sql_db.close_db(old_name)
    ...
    old_fs = etwork.tools.config.filestore(old_name)
    new_fs = etwork.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    etwork.tools.config.set_admin_password(new_password)
    etwork.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        etwork.tools.config['update']['base'] = True
        etwork.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@etwork.tools.mute_logger('etwork.sql_db')
...
def list_dbs(force=False):
    if not etwork.tools.config['list_db'] and not force:
        raise etwork.exceptions.AccessDenied()
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [etwork.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if etwork.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        etwork.sql_db.close_db(database_name)
...
def exp_list_lang():
    return etwork.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(etwork.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
