...
import etwork
from etwork.modules import get_modules
from etwork.modules.registry import Registry
from etwork.release import nt_service_name
from etwork.tools import config
from etwork.tools import stripped_sys_argv, dumpstacks, log_ormcache_stats
...
class RequestHandler(werkzeug.serving.WSGIRequestHandler):
    def setup(self):
        ...
        me.name = 'etwork.service.http.request.%s' % (me.ident,)
    ...
    def handle_file(self, path):
        if path.endswith('.py') and not os.path.basename(path).startswith('.~'):
            try:
                ...
                if not getattr(etwork, 'phoenix', False):
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in etwork.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="etwork.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        from etwork.addons.base.models.ir_cron import ir_cron
        while True:
            time.sleep(SLEEP_INTERVAL + number)     # Steve Reich timing style
            registries = etwork.modules.registry.Registry.registries
    ...
    def cron_spawn(self):
        ...
        for i in range(etwork.tools.config['max_cron_threads']):
            def target():
                self.cron_thread(i)
            t = threading.Thread(target=target, name="etwork.service.cron.cron%d" % i)
            t.setDaemon(True)
            t.type = 'cron'
            t.start()
    ...
    def http_spawn(self):
        t = threading.Thread(target=self.http_thread, name="etwork.service.httpd")
        t.setDaemon(True)
        t.start()
    ...
    def stop(self):
        ...
        """
        if getattr(etwork, 'phoenix', None):
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                logger = etwork.tests.runner._logger
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('etwork: %s %s %s' % (self.__class__.__name__, self.pid, title))
    ...
    def run(self):
        try:
            ...
            _logger.info("Worker (%s) exiting. request_count: %s, registry count: %s.",
                         ...
                         len(etwork.modules.registry.Registry.registries))
            self.stop()
        except Exception:
...
class WorkerCron(Worker):
    ...
    def _db_list(self):
        if config['db_name']:
            ...
            db_names = etwork.service.db.list_dbs(True)
        return db_names

    def process_work(self):
        rpc_request = logging.getLogger('etwork.netsvc.rpc.request')
        ...
        if len(db_names):
            ...
            from etwork.addons import base
...
def load_server_wide_modules():
    server_wide_modules = {'base', 'web'} | set(etwork.conf.server_wide_modules)
    for m in server_wide_modules:
        try:
            etwork.modules.module.load_openerp_module(m)
        except Exception:
            msg = ''
            if m == 'web':
                msg = """
...
def _reexec(updated_modules=None):
    ...
    if etwork.tools.osutil.is_running_as_nt_service():
        ...
        try:
            ...
            if config['test_file']:
                ...
                if not os.path.isfile(test_file):
                    ...
                    with etwork.api.Environment.manage():
                        load_test_file_py(registry, test_file)

            # run post-install tests
            if config['test_enable']:
                t0 = time.time()
                t0_sql = etwork.sql_db.sql_counter
                ...
                _logger.info("%d post-tests in %.2fs, %s queries",
                             ...
                             time.time() - t0,
                             etwork.sql_db.sql_counter - t0_sql)
...
def start(preload=None, stop=False):
    """ Start the etwork http server and cron processor.
    """
    global server
    ...
    if etwork.evented:
        server = GeventServer(etwork.service.wsgi_server.application)
    elif config['workers']:
        if config['test_enable'] or config['test_file']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(etwork.service.wsgi_server.application)
        ...
        server = ThreadedServer(etwork.service.wsgi_server.application)

    watcher = None
    if 'reload' in config['dev_mode'] and not etwork.evented:
        if inotify:
    ...
    if getattr(etwork, 'phoenix', False):
        _reexec()
