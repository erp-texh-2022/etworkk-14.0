...
#----------------------------------------------------------
# etwork must be a namespace package for etwork.addons to become one too
...
import sys
assert sys.version_info > (3, 6), "Outdated python version detected, etwork requires Python >= 3.6 to run."
...
if len(sys.argv) > 1 and sys.argv[1] == 'gevent':
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in etwork/debian/copyright)
        while 1:
            state = conn.poll()
...
from . import fields
from . import api
from etwork.tools.translate import _, _lt
