...
# *broken*
NO_POSTMORTEM = (etwork.exceptions.AccessDenied,
                 etwork.exceptions.UserError,
                 etwork.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                etwork.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = etwork.service.common.dispatch
        elif service_name == 'db':
            dispatch = etwork.service.db.dispatch
        elif service_name == 'object':
            dispatch = etwork.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                etwork.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                etwork.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except Exception as e:
        _logger.exception(etwork.tools.exception_to_unicode(e))
        etwork.tools.debugger.post_mortem(etwork.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in etwork.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~etwork.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~etwork.api.Environment` bound to current request. """
        if self._env is None:
            self._env = etwork.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            etwork.tools.debugger.post_mortem(
                etwork.tools.config, sys.exc_info())
    ...
    def registry(self):
        ...
        .. deprecated:: 8.0
            ...
            use :attr:`.env`
        """
        return etwork.registry(self.db)
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <etwork.http.WebRequest.csrf_token`, the
          :data:`~etwork.http.request` object is available by default
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, SessionExpiredException):
                ...
                elif isinstance(exception, (etwork.exceptions.UserError,
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["etwork", "addons"]:
            module = ""
        else:
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('etwork.addons.') and c.__module__.split(".")[2] in modules
...
class OpenERPSession(sessions.Session):
    ...
    def authenticate(self, db, login=None, password=None):
        ...
        uid = etwork.registry(db)['res.users'].authenticate(db, login, password, env)
        self.pre_uid = uid
    ...
    def check_security(self):
        ...
        env = etwork.api.Environment(request.cr, self.uid, self.context)
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or etwork.SUPERUSER_ID)
...
class Root(object):
    ...
    def nodb_routing_map(self):
        ...
        for url, endpoint, routing in etwork.http._generate_routing_rules([''] + etwork.conf.server_wide_modules, True):
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in etwork.addons.__path__:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        etwork.registry(db).check_signaling()
                        with etwork.tools.mute_logger('etwork.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = etwork.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if etwork.tools.config['dbfilter']:
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = etwork.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = etwork.tools.ustr(filename)
