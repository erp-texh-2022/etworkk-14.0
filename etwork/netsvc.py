...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('etwork.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', r'^Sampling from a set', category=DeprecationWarning, module='etwork')
...
DEFAULT_LOG_CONFIGURATION = [
    'etwork.http.rpc.request:INFO',
    'etwork.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['etwork:DEBUG', 'etwork.sql_db:INFO', 'etwork.http.rpc:DEBUG'],
    'debug_rpc': ['etwork:DEBUG', 'etwork.sql_db:INFO', 'etwork.http.rpc.request:DEBUG'],
    'debug': ['etwork:DEBUG', 'etwork.sql_db:INFO'],
    'debug_sql': ['etwork.sql_db:DEBUG'],
    'info': [],
    'runbot': ['etwork:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['etwork:WARNING', 'werkzeug:WARNING'],
    'error': ['etwork:ERROR', 'werkzeug:ERROR'],
    'critical': ['etwork:CRITICAL', 'werkzeug:CRITICAL'],
}
