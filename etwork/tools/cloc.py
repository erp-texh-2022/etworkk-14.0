...
import etwork
from etwork.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in etwork.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [etwork.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(etwork.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be etwork/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        with etwork.api.Environment.manage():
            registry = etwork.registry(config['db_name'])
            with registry.cursor() as cr:
                uid = etwork.SUPERUSER_ID
                env = etwork.api.Environment(cr, uid, {})
                self.count_env(env)
