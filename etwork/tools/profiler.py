...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if not isinstance(in_self, etwork.models.BaseModel):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all etwork models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non etwork model from the log: [None])
        ...
        .. code-block:: python
          ...
          from etwork.tools.profiler import profile
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @profile()                  # log all methods for all etwork models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/openerp/etwork/etwork/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _etworkProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_etworkProfile, method)

    wrapper = decorator(_etworkProfile, method)
    return wrapper
