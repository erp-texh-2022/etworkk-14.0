...
class GettextAlias(object):
    ...
    def _get_db(self):
        ...
        if db_name:
            return etwork.sql_db.db_connect(db_name)
...
def _extract_translatable_qweb_terms(element, callback):
    ...
    for el in element:
        ...
        if (el.tag.lower() not in SKIPPED_ELEMENTS
            ...
            # https://github.com/etwork/owl/blob/master/doc/reference/component.md#composition
...
class TranslationModuleReader:
    ...
    def __init__(self, cr, modules=None, lang=None):
        ...
        self._lang = lang
        self.env = etwork.api.Environment(cr, etwork.SUPERUSER_ID, {})
        self._to_translate = []
        self._path_list = [(path, True) for path in etwork.addons.__path__]
            ...
            for root, dummy, files in os.walk(path, followlinks=True):
                ...
                if fnmatch.fnmatch(root, '*/static/src*'):
                    ...
                    for fname in fnmatch.filter(files, '*.xml'):
                        self._babel_extract_terms(fname, path, root, 'etwork.tools.translate:babel_extract_qweb',
                                                  extra_comments=[WEB_TRANSLATION_COMMENT])
