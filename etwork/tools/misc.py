...
import etwork
import etwork.addons
# get_encodings, ustr and exception_to_unicode were originally from tools.misc.
# There are moved to loglevels until we refactor tools.
from etwork.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if etwork.tools.config['db_host']:
        env['PGHOST'] = etwork.tools.config['db_host']
    if etwork.tools.config['db_port']:
        env['PGPORT'] = str(etwork.tools.config['db_port'])
    if etwork.tools.config['db_user']:
        env['PGUSER'] = etwork.tools.config['db_user']
    if etwork.tools.config['db_password']:
        env['PGPASSWORD'] = etwork.tools.config['db_password']
    return env
...
def _fileopen(path, mode, basedir, pathinfo, basename=None, filter_ext=None, env=None):
    ...
    paths = etwork.addons.__path__ + [config['root_path']]
    if env and hasattr(env.all, '__file_open_tmp_paths'):
...
def file_open_temporary_directory(env):
    ...
    Examples::
        ...
        >>> with etwork.tools.file_open_temporary_directory(self.env) as module_dir:
        ...    with zipfile.ZipFile('foo.zip', 'r') as z:
        ...        z.extract('foo/__manifest__.py', module_dir)
        ...    with etwork.tools.file_open('foo/__manifest__.py', env=self.env) as f:
        ...        manifest = f.read()
...
def scan_languages():
    ...
    :rtype: [(str, unicode)]
    """
    csvpath = etwork.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('etwork.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('etwork.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, str):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = etwork.fields.Datetime.from_string(value)
            value = etwork.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = etwork.fields.Datetime.from_string(value)
...
def format_datetime(env, value, tz=False, dt_format='medium', lang_code=False):
    ...
    if not value:
        return ''
    if isinstance(value, str):
        timestamp = etwork.fields.Datetime.from_string(value)
    else:
        timestamp = value
