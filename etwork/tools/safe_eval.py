...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    except etwork.exceptions.UserError:
        raise
    except etwork.exceptions.RedirectWarning:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except etwork.http.AuthenticationError:
        raise
...
`etwork.tools.safe_eval.wrap_module` to generate a wrapper recursively
whitelisting allowed attributes.

Pre-wrapped modules are provided as attributes of `etwork.tools.safe_eval`.
""")
    return d
