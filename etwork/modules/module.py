...
import etwork
import etwork.tools as tools
import etwork.release as release
from etwork.tools import pycompat
...
# ad_paths is a deprecated alias, please use etwork.addons.__path__
@tools.lazy
def ad_paths():
    warnings.warn(
        '"etwork.modules.module.ad_paths" is a deprecated proxy to '
        '"etwork.addons.__path__".', DeprecationWarning, stacklevel=2)
    return etwork.addons.__path__
...
class AddonsHook(object):
    ...
    def find_module(self, name, path=None):
        if name.startswith('openerp.addons.') and name.count('.') == 2:
            warnings.warn(
                '"openerp.addons" is a deprecated alias to "etwork.addons".',
    ...
    def load_module(self, name):
        ...
        etwork_name = re.sub(r'^openerp.addons.(\w+)$', r'etwork.addons.\g<1>', name)

        etwork_module = sys.modules.get(etwork_name)
        if not etwork_module:
            etwork_module = importlib.import_module(etwork_name)

        sys.modules[name] = etwork_module
...
class etworkHook(object):
    ...
    def find_module(self, name, path=None):
        ...
        if re.match(r'^openerp\b', name):
            warnings.warn(
                'openerp is a deprecated alias to etwork.',
                DeprecationWarning, stacklevel=2)
            return self
    ...
    def load_module(self, name):
        ...
        canonical = re.sub(r'^openerp(.*)', r'etwork\g<1>', name)
...
class UpgradeHook(object):
    """Makes the legacy `migrations` package being `etwork.upgrade`"""

    def find_module(self, name, path=None):
        if re.match(r"^etwork.addons.base.maintenance.migrations\b", name):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("etwork.addons.base.maintenance.migrations", "etwork.upgrade")
...
def initialize_sys_path():
    """
    Setup the addons path ``etwork.addons.__path__`` with various defaults
    and explicit directories.
    """
    # hook etwork.addons on data dir
    dd = os.path.normcase(tools.config.addons_data_dir)
    if os.access(dd, os.R_OK) and dd not in etwork.addons.__path__:
        etwork.addons.__path__.append(dd)

    # hook etwork.addons on addons paths
    for ad in tools.config['addons_path'].split(','):
        ad = os.path.normcase(os.path.abspath(tools.ustr(ad.strip())))
        if ad not in etwork.addons.__path__:
            etwork.addons.__path__.append(ad)

    # hook etwork.addons on base module path
    base_path = os.path.normcase(os.path.abspath(os.path.join(os.path.dirname(os.path.dirname(__file__)), 'addons')))
    if base_path not in etwork.addons.__path__ and os.path.isdir(base_path):
        etwork.addons.__path__.append(base_path)

    # hook etwork.upgrade on upgrade-path
    from etwork import upgrade
    ...
    # create decrecated module alias from etwork.addons.base.maintenance.migrations to etwork.upgrade
    spec = importlib.machinery.ModuleSpec("etwork.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = upgrade
    sys.modules["etwork.addons.base.maintenance"] = maintenance_pkg
    sys.modules["etwork.addons.base.maintenance.migrations"] = upgrade
...
def get_module_path(module, downloaded=False, display_warning=True):
    ...
    """
    for adp in etwork.addons.__path__:
...
def get_module_filetree(module, dir='.'):
    ...
    files = etwork.tools.osutil.listdir(path, True)
...
def get_resource_from_path(path):
    ...
    """
    resource = False
    for adpath in etwork.addons.__path__:
...
def load_openerp_module(module_name):
    ...
    try:
        __import__('etwork.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['etwork.addons.' + module_name], info['post_load'])()
...
def get_modules():
    ...
    plist = []
    for ad in etwork.addons.__path__:
