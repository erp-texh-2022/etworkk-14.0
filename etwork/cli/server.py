...
__author__ = etwork.release.author
__version__ = etwork.release.version

# Also use the `etwork` logger for the main script.
_logger = logging.getLogger('etwork')
...
def setup_pid_file():
    ...
    """
    config = etwork.tools.config
    if not etwork.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = etwork.modules.registry.Registry.new(dbname)
        with etwork.api.Environment.manage():
            with registry.cursor() as cr:
                etwork.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with etwork.api.Environment.manage():
        with registry.cursor() as cr:
            etwork.tools.trans_load(
                cr, config["translate_in"], config["language"], overwrite=overwrite,
            )

def main(args):
    check_root_user()
    etwork.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                etwork.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except etwork.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        etwork.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = etwork.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the etwork server (default command)"""
    def run(self, args):
        main(args)
