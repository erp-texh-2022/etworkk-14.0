...
class Shell(Command):
    """Start etwork in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        etwork.cli.server.report_configuration()
        etwork.service.server.start(preload=[], stop=True)
        ...
        with etwork.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = etwork.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = etwork.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
