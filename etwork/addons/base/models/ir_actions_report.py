...
from etwork.exceptions import UserError, AccessError
from etwork.tools.safe_eval import safe_eval, time
from etwork.tools.misc import find_in_path, ustr
from etwork.tools import config
from etwork.sql_db import TestCursor
from etwork.http import request
from etwork.osv.expression import NEGATIVE_TERM_OPERATORS, FALSE_DOMAIN
...
# here to init the T1 fonts cache at the start-up of etwork so that rendering of barcode in multiple
...
# Check the presence of Wkhtmltopdf and return its version at etwork start-up
...
try:
    ...
    if match:
        ...
        if config['workers'] == 1:
            _logger.info('You need to start etwork with at least two workers to print a pdf version of the reports.')
...
class IrActionsReport(models.Model):
    ...
    def _raise_on_unreadable_pdfs(self, streams, stream_record):
        ...
        if unreadable_pdfs:
            ...
            raise UserError(_(
                "etwork is unable to merge the PDFs attached to the following records:\n"
                "%s\n\n"
