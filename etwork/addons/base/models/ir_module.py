...
import etwork
from etwork import api, fields, models, modules, tools, _
from etwork.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from etwork.exceptions import AccessDenied, UserError
from etwork.osv import expression
from etwork.tools.parse_version import parse_version
from etwork.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'etwork Enterprise Edition License v1.0'),
        ('OPL-1', 'etwork Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('etwork Enterprise Module', default=False)
    ...
    def _button_immediate_function(self, function):
        ...
        try:
            ...
            raise UserError(_("etwork is currently processing a scheduled action.\n"
    ...
    def button_uninstall(self):
        un_installable_modules = set(etwork.conf.server_wide_modules) & set(self.mapped('name'))
    ...
    def install_from_urls(self, urls):
        ...
        OPENERP = etwork.release.product_name.lower()
        ...
        try:
            ...
            if urls.get(OPENERP):
                ...
                # copy all modules in the SERVER/etwork/addons directory to the new "etwork" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, OPENERP, 'addons', d)    # XXX 'etwork' subdirectory ?
                ...
                _logger.info('Copy downloaded module `etwork` to `%s`', server_dir)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.etwork.com/apps')
