...
from etwork.tests.common import BaseCase, SavepointCase, TransactionCase
from etwork.tools import (
...
class EmailConfigCase(SavepointCase):
    @patch.dict("etwork.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'etwork.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@etwork.example.com')
        set_param('mail.catchall.domain', 'etwork.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin" <notification@etwork.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@etwork.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@etwork.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@etwork.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@etwork.example.com>')
        set_param('mail.catchall.domain', 'etwork.example.com')
