...
from etwork.exceptions import AccessDenied, AccessError
from etwork.http import _request_stack

import etwork.tools
from etwork.tests import common
from etwork.service import common as auth, model
...
class TestXMLRPC(common.HttpCase):
    ...
    def _json_call(self, *args):
        self.opener.post("http://%s:%s/jsonrpc" % (common.HOST, etwork.tools.config['http_port']), json={
            'jsonrpc': '2.0',
            'id': None,
            'method': 'call',
            'params': {
