# -*- coding: utf-8 -*-
"""
The module :mod:`etwork.tests.common` provides unittest test cases and a few
...
import etwork
from etwork import api
from etwork.models import BaseModel
from etwork.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from etwork.service import security
from etwork.sql_db import Cursor
from etwork.tools import float_compare, single_email_re
from etwork.tools.misc import find_in_path
from etwork.tools.safe_eval import safe_eval
...
# The etwork library is supposed already configured.
ADDONS_PATH = etwork.tools.config['addons_path']
HOST = '127.0.0.1'
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = etwork.SUPERUSER_ID


def get_db_name():
    db = etwork.tools.config['db_name']
...
def standalone(*tags):
    ...
    def register(func):
        ...
        if func.__module__.startswith('etwork.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most etwork operations;
    """
    if not login:
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('etwork.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    ...
    longMessage = True      # more verbose error message by default: https://www.etwork.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~etwork.models.BaseModel`
        """
    ...
    def assertQueries(self, expected, flush=True):
        ...
        with patch('etwork.sql_db.Cursor.execute', execute):
            with patch('etwork.osv.expression.get_unaccent_wrapper', get_unaccent_wrapper):
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/etwork/addons/" in filename:
                        filename = filename.rsplit("/etwork/addons/", 1)[1]
                    if count > expected:
    ...
    def _callSetUp(self):
        ...
        # See https://github.com/etwork/etwork/pull/107572 for more info.
...
class _ErrorCatcher(list):
    ...
    def _complete_traceback(self, initial_tb):
        ...
        # remove traceback root part (etwork_bin, main, loading, ...), as
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = etwork.registry(get_db_name())
        ...
        #: :class:`~etwork.api.Environment` for the current test case
        self.env = api.Environment(self.cr, etwork.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = etwork.registry(get_db_name())
        ...
        cls.env = api.Environment(cls.cr, etwork.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger, window_size, test_class):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_etwork')
        self.chrome_pid = None

        otc = etwork.tools.config
...
class HttpCaseCommon(BaseCase):
    ...
    def __init__(self, methodName='runTest'):
        ...
        self.xmlrpc_url = url_8 = 'http://%s:%d/xmlrpc/2/' % (HOST, etwork.tools.config['http_port'])
    ...
    def url_open(self, url, data=None, files=None, timeout=10, headers=None, allow_redirects=True):
        self.env['base'].flush()
        if url.startswith('/'):
            url = "http://%s:%s%s" % (HOST, etwork.tools.config['http_port'], url)
        if data or files:
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('etwork.service.http.request.')]
        ...
        if request_threads:
            ...
            etwork.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=True)
        etwork.http.root.session_store.save(self.session)

    def authenticate(self, user, password):
        db = get_db_name()
        if getattr(self, 'session', None):
            etwork.http.root.session_store.delete(self.session)

        self.session = session  = etwork.http.root.session_store.new()
        session.db = db
        ...
        etwork.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, **kw):
        ...
        try:
            ...
            base_url = "http://%s:%s" % (HOST, etwork.tools.config['http_port'])
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        code = kwargs.pop('code', "etwork.startTour('%s'%s)" % (tour_name, step_delay))
        ready = kwargs.pop('ready', "etwork.__DEBUG__.services['web_tour.tour'].tours['%s'].ready" % tour_name)
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~etwork.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~etwork.fields.Many2many` fields, the field itself is a
    :class:`~etwork.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~etwork.fields.One2many` are reified as
    :class:`~etwork.tests.common.O2MProxy`.

    Because the :class:`~etwork.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~etwork.tests.common.O2MProxy.new` and
    :meth:`~etwork.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: etwork.models.Model
    ...
    :type view: int | str | etwork.model.Model
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~etwork.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~etwork.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, v in record.read(to_read)[0].items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = etwork.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = etwork.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return etwork.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return etwork.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from etwork.tests.common have a test_tags
