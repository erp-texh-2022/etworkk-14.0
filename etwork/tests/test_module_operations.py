...
import etwork
from etwork.tools import config, topological_sort, unique
from etwork.netsvc import init_logger
from etwork.tests import standalone_tests
import etwork.tests.loader

_logger = logging.getLogger('etwork.tests.test_module_operations')
...
def install(db_name, module_id, module_name):
    with etwork.api.Environment.manage():
        with etwork.registry(db_name).cursor() as cr:
            env = etwork.api.Environment(cr, etwork.SUPERUSER_ID, {})
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall cycle of etwork modules")
    ...
    parser.add_argument("--addons-path", "-p", type=str, action=CheckAddons,
        help="Comma-separated list of paths to directories containing extra etwork modules")
...
def test_full(args):
    ...
    with etwork.api.Environment.manage():
        with etwork.registry(args.database).cursor() as cr:
            env = etwork.api.Environment(cr, etwork.SUPERUSER_ID, {})
...
def test_scripts(args):
    ...
    registry = etwork.registry(args.database)
    for module_name in registry._init_modules:
        # import tests for loaded modules
        etwork.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    # handle paths option
    if args.addons_path:
        etwork.tools.config['addons_path'] = ','.join([args.addons_path, etwork.tools.config['addons_path']])
        if args.data_dir:
            etwork.tools.config['data_dir'] = args.data_dir
        etwork.modules.module.initialize_sys_path()

    init_logger()
    logging.getLogger('etwork.modules.loading').setLevel(logging.CRITICAL)
    logging.getLogger('etwork.sql_db').setLevel(logging.CRITICAL)
