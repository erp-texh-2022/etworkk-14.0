...
from etwork.tools.translate import translate, translate_sql_constraint
...
def dispatch(method, params):
    ...
    registry = etwork.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            with closing(etwork.sql_db.db_connect(dbname).cursor()) as cr:
        ...
        while True:
            try:
                if etwork.registry(dbname)._init and not etwork.tools.config['test_enable']:
                    raise etwork.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    etwork.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = etwork.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist", obj))
    result = etwork.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with etwork.registry(db).cursor() as cr:
