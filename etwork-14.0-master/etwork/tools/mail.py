...
import etwork
from etwork.loglevels import ustr
from etwork.tools import misc
...
def email_send(email_from, email_to, subject, body, email_cc=None, email_bcc=None, reply_to=False,
    ...
    if not cr:
        ...
        if db_name:
            local_cr = cr = etwork.registry(db_name).cursor()
        else:
            raise Exception("No database cursor found, please pass one explicitly")

    # Send Email
    try:
        mail_server_pool = etwork.registry(cr.dbname)['ir.mail_server']
        res = False
...
def encapsulate_email(old_email, new_email):
    ...
    * New From: notifications@etwork.com
    * Output: "Admin" <notifications@etwork.com>
    """
