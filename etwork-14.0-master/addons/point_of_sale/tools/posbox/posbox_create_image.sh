...
REPO=https://github.com/etwork/etwork.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/etwork"
...
etwork/
etwork-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/etwork
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/etwork/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
