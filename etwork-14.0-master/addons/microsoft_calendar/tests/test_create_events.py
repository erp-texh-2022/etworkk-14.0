...
from etwork.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        ...
        self.assert_etwork_event(new_records, self.expected_etwork_event_from_outlook)
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_etwork(
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@etwork.com", 'name': "John Doe"},
        })
        expected_event = dict(self.expected_etwork_event_from_outlook, user_id=False)
        ...
        new_records = (records - existing_records)
        self.assertEqual(len(new_records), 1)
        self.assert_etwork_event(new_records, expected_event)
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        ...
        self.assert_etwork_recurrence(new_recurrences, self.expected_etwork_recurrency_from_outlook)
        for i, e in enumerate(sorted(new_events, key=lambda e: e.id)):
            self.assert_etwork_event(e, self.expected_etwork_recurrency_events_from_outlook[i])
