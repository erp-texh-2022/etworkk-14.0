...
from etwork.addons.mass_mailing.tests.common import MassMailCommon
from etwork.tests.common import users, Form, tagged
from etwork.tools import formataddr, mute_logger
...
class TestMassMailFeatures(MassMailCommon):
    ...
    def test_mailing_shortener(self):
        mailing = self.env['mailing.mailing'].create({
            ...
            'body_html': """<div>
Hi,
% set url = "www.etwork.com"
% set httpurl = "https://www.etwork.eu"
Website0: <a id="url0" href="https://www.etwork.tz/my/${object.name}">https://www.etwork.tz/my/${object.name}</a>
Website1: <a id="url1" href="https://www.etwork.be">https://www.etwork.be</a>
...
Email: <a id="url5" href="mailto:test@etwork.com">test@etwork.com</a></div>""",
            'mailing_model_id': self.env['ir.model']._get('mailing.list').id,
            'reply_to_mode': 'email',
            'reply_to': self.email_reply_to,
            'contact_list_ids': [(6, 0, self.mailing_list_1.ids)],
        ...
        for contact in self.mailing_list_1.contact_ids:
            ...
            for link_info in [('url0', 'https://www.etwork.tz/my/%s' % contact.name, True),
                              ('url1', 'https://www.etwork.be', True),
                              ('url2', 'https://www.etwork.com', True),
                              ('url3', 'https://www.etwork.eu', True),
                              ('url4', 'https://www.example.com/foo/bar?baz=qux', True),
                              ('url5', 'mailto:test@etwork.com', False)]:
