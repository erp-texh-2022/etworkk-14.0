...
from etwork.addons.payment.tests.common import PaymentAcquirerCommon
from etwork.addons.payment_authorize.controllers.main import AuthorizeController
...
@etwork.tests.tagged('post_install', '-at_install', '-standard', 'external')
class AuthorizeForm(AuthorizeCommon):
    ...
    @mute_logger('etwork.addons.payment_authorize.models.payment', 'ValidationError')
...
@etwork.tests.tagged('post_install', '-at_install', '-standard')
class AuthorizeS2s(AuthorizeCommon):
    def test_30_authorize_s2s(self):
        ...
        # add credential
        # FIXME: put this test in master-nightly on etwork/etwork + create sandbox account
        authorize.write({
