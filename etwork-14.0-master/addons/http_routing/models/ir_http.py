...
import etwork
from etwork import api, models, registry, exceptions, tools, http
from etwork.addons.base.models.ir_http import RequestUID, ModelConverter
from etwork.addons.base.models.qweb import QWebException
from etwork.http import request
from etwork.osv import expression
from etwork.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if etwork._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            etwork._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if etwork._geoip_resolver and request.httprequest.remote_addr:
                record = etwork._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except etwork.exceptions.MissingError:
