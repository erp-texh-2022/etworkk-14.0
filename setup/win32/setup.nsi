...
!ifndef SERVICENAME
	!define SERVICENAME 'etwork-server-${VERSION}'
!endif
...
OutFile "etwork_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\etwork-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\etwork-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\etwork-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\etwork-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.etwork.com/page/contactus"
...
Section $(TITLE_Dosyt_Server) SectionDosyt_Server
    ...
    WriteIniStr "$INSTDIR\server\etwork.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\etwork.conf" "options" "addons_path" "$INSTDIR\server\etwork\addons"
    WriteIniStr "$INSTDIR\server\etwork.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\etwork.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\etwork-bin" --stop-after-init --logfile "$INSTDIR\server\etwork.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\etwork-bin\""'
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@etwork.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.etwork.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@etwork.com"
