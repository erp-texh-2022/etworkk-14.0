...
from etwork.addons.test_mail.tests.common import TestMailCommon, TestRecipients
...
class Testetworkbot(TestMailCommon, TestRecipients):
    ...
    def test_fetch_listener(self):
        channel = self.env['mail.channel'].with_user(self.user_employee).init_etworkbot()
        partners = self.env['mail.channel'].channel_fetch_listeners(channel.uuid)
        etworkbot = self.env.ref("base.partner_root")
        etworkbot_in_fetch_listeners = [partner for partner in partners if partner['id'] == etworkbot.id]
        self.assertEqual(len(etworkbot_in_fetch_listeners), 1, 'etworkbot should appear only once in channel_fetch_listeners')

    @mute_logger('etwork.addons.mail.models.mail_mail')
    def test_etworkbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.etworkbot_ping_body, 'partner_ids': [self.etworkbot.id, self.user_admin.partner_id.id]})
    ...
    def test_onboarding_flow(self):
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.etworkbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.etworkbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('etwork.addons.mail.models.mail_mail')
    def test_etworkbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @etworkbot right now", 'partner_ids': []})
        self.assertNextMessage(
