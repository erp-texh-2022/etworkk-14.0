...
from etwork import _, models
from etwork.tools import float_repr
...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_fill_invoice_line_values(self, tree, xpath_dict, invoice_line_form, qty_factor):
        """
        Read the xml invoice, extract the invoice line values, compute the etwork values
        ...
        if quantity_node is not None:
            ...
            if uom_xml:
                uom_infered_xmlid = [
                    etwork_xmlid for etwork_xmlid, uom_unece in UOM_TO_UNECE_CODE.items() if uom_unece == uom_xml
                ]
