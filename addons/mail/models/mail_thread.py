...
from etwork import _, api, exceptions, fields, models, tools, registry, SUPERUSER_ID
from etwork.exceptions import MissingError
from etwork.osv import expression

from etwork.tools import ustr
from etwork.tools.misc import clean_context, split_every
...
class MailThread(models.AbstractModel):
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            if thread._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # parsing should find an author independently of user running mail gateway, and ensure it is not etworkbot
    ...
    def message_post(self, *,
        ...
        if values['author_id'] and values['message_type'] != 'notification' and not self._context.get('mail_create_nosubscribe'):
            if self.env['res.partner'].browse(values['author_id']).active:  # we dont want to add etworkbot/inactive as a follower
