etwork.define('web.ErrorDialogRegistry', function (require) {
"use strict";
...
etwork.define('web.CrashManager', function (require) {
"use strict";
...
var CrashManager = AbstractService.extend({
    init: function () {
        ...
        this.etworkExceptionTitleMap = {
            'etwork.addons.base.models.ir_mail_server.MailDeliveryException': _lt("MailDeliveryException"),
            'etwork.exceptions.AccessDenied': _lt("Access Denied"),
            'etwork.exceptions.AccessError': _lt("Access Error"),
            'etwork.exceptions.MissingError': _lt("Missing Record"),
            'etwork.exceptions.UserError': _lt("User Error"),
            'etwork.exceptions.ValidationError': _lt("Validation Error"),
            'etwork.exceptions.Warning': _lt("Warning"),
        };
    ...
    rpc_error: function(error) {
        ...
        // etwork custom exception: UserError, AccessError, ...
        if (_.has(this.etworkExceptionTitleMap, error.data.name)) {
            error = _.extend({}, error, {
                data: _.extend({}, error.data, {
                    message: error.data.arguments[0],
                    title: this.etworkExceptionTitleMap[error.data.name],
                }),
            });
    ...
    show_error: function (error) {
        ...
        var dialog = new dialogClass(this, {
            title: _.str.capitalize(error.type) || _t("etwork Error"),
        }, error);
...
var RedirectWarningHandler = Widget.extend(ExceptionHandler, {
    ...
    display: function() {
        ...
        new WarningDialog(this, {
            title: _.str.capitalize(error.type) || _t("etwork Warning"),
            buttons: [
...
core.crash_registry.add('etwork.exceptions.RedirectWarning', RedirectWarningHandler);

function session_expired(cm) {
    return {
        display: function () {
            const notif = {
                type: _t("etwork Session Expired"),
                message: _t("Your etwork session expired. The current page is about to be refreshed."),
            };
            const options = {
                buttons: [{
                    text: _t("Ok"),
...
core.crash_registry.add('etwork.http.SessionExpiredException', session_expired);
