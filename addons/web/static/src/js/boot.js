...
(function () {
    ...
    etwork.testing = typeof QUnit === 'object';
    etwork.remainingJobs = jobs;
    etwork.__DEBUG__ = {
    ...
    etwork.define = function () {
        var args = Array.prototype.slice.call(arguments);
        var name = typeof args[0] === 'string' ? args.shift() : ('__etwork_job' + (jobUID++));
        ...
        if (jobs.length) {
            ...
            for (var k = 0; k < jobs.length; k++) {
                debugJobs[jobs[k].name] = job = {
                    ...
                    dependents: etwork.__DEBUG__.getDependents(jobs[k].name),
                    name: jobs[k].name
                };
                if (jobs[k].error) {
                ...
                var deps = etwork.__DEBUG__.getDependencies(job.name);
            ...
            missing = etwork.__DEBUG__.getMissingJobs();
            failed = etwork.__DEBUG__.getFailedJobs();
            ...
            if (etwork.debug || failed.length || unloaded.length) {
                ...
                if (etwork.debug && Object.keys(debugJobs).length) {
                    log('Debug:                   ', debugJobs);
                }
            }
        }
        etwork.__DEBUG__.jsModules = {
            missing: missing,
    ...
    etwork.processJobs = function (jobs, services) {
        var job;
                ...
                if (!job.error) {
                    Promise.resolve(jobExec).then(
                        function (data) {
                            ...
                            resolve();
                            etwork.processJobs(jobs, services);
