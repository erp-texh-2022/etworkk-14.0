...
class MicrosoftEvent(abc.Set):
    ...
    def _load_etwork_ids_from_db(self, env, force_model=None):
        ...
        """
        mapped_events = [e.id for e in self if e._etwork_id]
        ...
        etwork_events = model_env.with_context(active_test=False).search([
            '|',
        ...
        for ms_event in unmapped_events_with_uids:
            etwork_id = mapping.get(ms_event.iCalUId)
            if etwork_id:
        ...
        for ms_event in unmapped_events:
            etwork_event = mapping.get(ms_event.id)
            if etwork_event:
    ...
    def match_with_etwork_events(self, env) -> 'MicrosoftEvent':
        ...
        mapped_recurrences = recurrence_candidates._load_etwork_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_etwork_ids_from_db(env)
