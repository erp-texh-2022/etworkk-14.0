...
from etwork import api, fields, models, registry
from etwork.tools import ormcache_context
from etwork.exceptions import UserError
from etwork.osv import expression

from etwork.addons.microsoft_calendar.utils.microsoft_event import MicrosoftEvent
from etwork.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from etwork.addons.microsoft_calendar.utils.event_id_storage import IDS_SEPARATOR, combine_ids, split_ids
from etwork.addons.microsoft_account.models.microsoft_service import TIMEOUT
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_etwork2microsoft(self):
        if not self:
            return
    ...
    def _sync_recurrence_microsoft2etwork(self, microsoft_events, new_events=None):
        ...
        for recurrent_master in recurrent_masters:
            new_calendar_recurrence = dict(
                self.env['calendar.recurrence']._microsoft_to_etwork_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            ...
            base_values = dict(
                self.env['calendar.event']._microsoft_to_etwork_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            to_create_values = []
            ...
            for recurrent_event in to_create:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_etwork_recurrence_values(recurrent_event, base_values)
                else:
                    value = self.env['calendar.event']._microsoft_to_etwork_values(recurrent_event, default_values)
            ...
            new_recurrence_etwork = self.env['calendar.recurrence'].create(new_calendar_recurrence)
            new_recurrence_etwork.base_event_id = new_recurrence_etwork.calendar_event_ids[0] if new_recurrence_etwork.calendar_event_ids else False
            new_recurrence |= new_recurrence_etwork
        ...
        for recurrent_master_id in ms_recurrence_ids:
            ...
            for recurrent_event in to_update:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_etwork_recurrence_values(
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        ...
        for e in events_to_update:
            if e.type == "exception":
                event_values = self.env['calendar.event']._microsoft_to_etwork_values(e)
            elif e.type == "occurrence":
                event_values = self.env['calendar.event']._microsoft_to_etwork_recurrence_values(e)
            else:
                event_values = None
            ...
            if event_values:
                ...
                etwork_event = self.env['calendar.event'].browse(e.etwork_id(self.env)).exists().with_context(
                    no_mail_to_attendees=True, mail_create_nolog=True
                )
                etwork_event.write(dict(event_values, need_sync_m=False))
                update_events |= etwork_event
    ...
    def _sync_microsoft2etwork(self, microsoft_events: MicrosoftEvent):
        ...
        """
        existing = microsoft_events.match_with_etwork_events(self.env)
        ...
        etwork_values = [
            dict(self._microsoft_to_etwork_values(e, with_ids=True), need_sync_m=False)
            for e in (new - new_recurrence)
        ]
        synced_events = self.with_context(dont_notify=True)._create_from_microsoft(new, etwork_values)
        synced_recurrences, updated_events = self._sync_recurrence_microsoft2etwork(existing, new_recurrence)
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            # Last updated wins.
            # This could be dangerous if microsoft server time and etwork server time are different
            if mevent.is_recurrence():
                etwork_event = self.env['calendar.recurrence'].browse(mevent.etwork_id(self.env)).exists()
            else:
                etwork_event = self.browse(mevent.etwork_id(self.env)).exists()

            if etwork_event:
                etwork_event_updated_time = pytz.utc.localize(etwork_event.write_date)
                ms_event_updated_time = parse(mevent.lastModifiedDateTime)

                if ms_event_updated_time >= etwork_event_updated_time:
                    vals = dict(etwork_event._microsoft_to_etwork_values(mevent), need_sync_m=False)
                    etwork_event._write_from_microsoft(mevent, vals)

                    if etwork_event._name == 'calendar.recurrence':
                        update_events = etwork_event._update_microsoft_recurrence(mevent, microsoft_events)
                        synced_recurrences |= etwork_event
                        synced_events |= update_events
                    else:
                        synced_events |= etwork_event
    ...
    @api.model
    def _microsoft_to_etwork_values(
