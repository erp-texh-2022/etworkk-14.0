...
from etwork.tests.common import HttpCase

from etwork.addons.microsoft_calendar.models.microsoft_sync import MicrosoftSync
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        self.expected_etwork_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_etwork_recurrency_events_from_outlook = [
            {
    ...
    def assert_etwork_event(self, etwork_event, expected_values):
        ...
        etwork_event_values = etwork_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(etwork_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, etwork_event_values.get(k), msg=f"'{k}' mismatch")
    ...
    def assert_etwork_recurrence(self, etwork_recurrence, expected_values):
        ...
        for k, v in expected_values.items():
            self.assertEqual(v, etwork_recurrence_values.get(k), msg=f"'{k}' mismatch")
