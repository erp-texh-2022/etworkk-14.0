...
from etwork.tools import consteq
from etwork.exceptions import ValidationError

from etwork.addons.payment_stripe.controllers.main import StripeController
...
class PaymentAcquirerStripe(models.Model):
    ...
    def _handle_checkout_webhook(self, checkout_object: dir):
        ...
        try:
            etwork_tx = self.env['payment.transaction']._stripe_form_get_tx_from_data(data)
        except ValidationError as e:
            _logger.info('Received notification for tx %s. Skipped it because of %s', tx_reference, e)
            return False

        PaymentAcquirerStripe._verify_stripe_signature(etwork_tx.acquirer_id)

        url = 'payment_intents/%s' % etwork_tx.stripe_payment_intent
        stripe_tx = etwork_tx.acquirer_id._stripe_request(url)
