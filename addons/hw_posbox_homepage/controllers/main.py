...
from etwork.http import Response
from etwork.modules.module import get_resource_path

from etwork.addons.hw_drivers.main import iot_devices
from etwork.addons.hw_drivers.tools import helpers
from etwork.addons.web.controllers import main as web
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('etwork.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/etwork/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])

    def get_six_terminal(self):
        terminal_id = helpers.read_file_first_line('etwork-six-payment-terminal.conf')
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_etwork_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'etwork-remote-server.conf'
    ...
    def load_iot_handlers(self):
        ...
        subprocess.check_call(["sudo", "service", "etwork", "restart"])
    ...
    def clear_credential(self):
        helpers.unlink_file('etwork-db-uuid.conf')
        helpers.unlink_file('etwork-enterprise-code.conf')
        subprocess.check_call(["sudo", "service", "etwork", "restart"])
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('etwork-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_etwork_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_etwork_server_url() or '',
    ...
    def add_six_payment_terminal(self, terminal_id):
        helpers.write_file('etwork-six-payment-terminal.conf', terminal_id)
        subprocess.check_call(["sudo", "service", "etwork", "restart"])
        return 'http://' + helpers.get_ip() + ':8069'

    @http.route('/six_payment_terminal_clear', type='http', auth='none', cors='*', csrf=False)
    def clear_six_payment_terminal(self):
        helpers.unlink_file('etwork-six-payment-terminal.conf')
        subprocess.check_call(["sudo", "service", "etwork", "restart"])
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"

    @http.route('/hw_proxy/upgrade', type='http', auth='none', )
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/etwork/", "--git-dir=/home/pi/etwork/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/etwork/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/etwork/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/etwork/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspios']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspios(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/etwork/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspios']).decode().split('\n')[-2]
