...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, record, values, command=None):
        ...
        The logic will only be applied if etworkbot is in a chat with a user or
        if someone pinged etworkbot.

         :param record: the mail_thread (or mail_channel) where the user
            message was posted/etworkbot will answer.
        ...
        if len(record) != 1 or values.get("author_id") == etworkbot_id:
            return
        if self._is_bot_pinged(values) or self._is_bot_in_private_channel(record):
            ...
            if answer:
                ...
                record.with_context(mail_create_nosubscribe=True).sudo().message_post(body=answer, author_id=etworkbot_id, message_type=message_type, subtype_id=subtype_id)
    ...
    def _get_answer(self, record, body, values, command=False):
        ...
        if self._is_bot_in_private_channel(record):
            # main flow
            if etworkbot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.etworkbot_state = "onboarding_command"
                self.env.user.etworkbot_failed = False
                return _("Great! 👍<br/>To access special commands, <b>start your sentence with</b> <span class=\"o_etworkbot_command\">/</span>. Try getting help.")
            ...
            # easter eggs
            elif etworkbot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or etworkbot_state == 'idle':
                return _("Unfortunately, I'm just a bot 😞 I don't understand! If you need help discovering our product, please check "
                         "<a href=\"https://www.etwork.com/page/docs\" target=\"_blank\">our documentation</a> or "
                         "<a href=\"https://www.etwork.com/slides\" target=\"_blank\">our videos</a>.")
            else:
                # repeat question
                if etworkbot_state == 'onboarding_emoji':
                    self.env.user.etworkbot_failed = True
                    return _("Not exactly. To continue the tour, send an emoji: <b>type</b> <span class=\"o_etworkbot_command\">:)</span> and press enter.")
                elif etworkbot_state == 'onboarding_attachement':
                    self.env.user.etworkbot_failed = True
                    return _("To <b>send an attachment</b>, click on the <i class=\"fa fa-paperclip\" aria-hidden=\"true\"></i> icon and select a file.")
                elif etworkbot_state == 'onboarding_command':
                    self.env.user.etworkbot_failed = True
                ...
                return random.choice([
                    ...
                    _("Sorry I'm sleepy. Or not! Maybe I'm just trying to hide my unawareness of human language...<br/>I can show you features if you write: <span class=\"o_etworkbot_command\">start the tour</span>.")
                ])
        return False
    ...
    def _is_bot_pinged(self, values):
        etworkbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        return etworkbot_id in values.get('partner_ids', [])

    def _is_bot_in_private_channel(self, record):
        etworkbot_id = self.env['ir.model.data'].xmlid_to_res_id("base.partner_root")
        if record._name == 'mail.channel' and record.channel_type == 'chat':
            return etworkbot_id in record.with_context(active_test=False).channel_partner_ids.ids
        return False
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.etworkbot_failed
