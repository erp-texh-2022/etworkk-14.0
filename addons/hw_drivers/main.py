...
class Manager(Thread):
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to etwork database
        """
        server = helpers.get_etwork_server_url()
        if server:
            subject = helpers.read_file_first_line('etwork-subject.conf')
            if subject:
            ...
            _logger.warning('etwork server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the etwork server with the updates
        """
