...
class IoTRestart(Thread):
    """
    Thread to restart etwork server in IoT Box when we must return a answer before
    """
...
def add_credential(db_uuid, enterprise_code):
    write_file('etwork-db-uuid.conf', db_uuid)
    write_file('etwork-enterprise-code.conf', enterprise_code)
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/etwork/', '--git-dir=/home/pi/etwork/.git']
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/etwork/addons/hw_drivers/iot_handlers/drivers/*"])
                    subprocess.check_call(["rm", "-rf", "/home/pi/etwork/addons/hw_drivers/iot_handlers/interfaces/*"])
...
def check_image():
    ...
    """
    url = 'https://nightly.etwork.com/master/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_etwork_server_url():
    ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
    if not ap:
        return False
    return read_file_first_line('etwork-remote-server.conf')

def get_token():
    return read_file_first_line('token')

def get_version():
    return subprocess.check_output(['cat', '/var/etwork/iotbox_version']).decode().rstrip()
...
def load_certificate():
    ...
    url = 'https://www.etwork.com/etwork-enterprise/iot/x509'
    data = {
        'params': {
            'db_uuid': db_uuid,
    ...
    try:
        ...
        _logger.exception("An error occurred while trying to reach etwork.com servers.")
    ...
    write_file('etwork-subject.conf', result['subject_cn'])
    subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_iot_handlers(auto=True):
    ...
    if server:
        ...
        try:
            ...
            if resp.data:
                subprocess.call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'etwork/addons/hw_drivers/iot_handlers'
...
def load_iot_handlers():
    """
    This method loads local files: 'etwork/addons/hw_drivers/iot_handlers/drivers' and
    'etwork/addons/hw_drivers/iot_handlers/interfaces'
