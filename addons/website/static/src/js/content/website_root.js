etwork.define('website.root', function (require) {
'use strict';
...
var WebsiteRoot = publicRootData.PublicRoot.extend(KeyboardNavigationMixin, {
    ...
    start: function () {
        ...
        this.$('.zoomable img[data-zoom]').zoometwork();
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                window.etwork_gmap_api_post_load = (async function etwork_gmap_api_post_load() {
                ...
                await ajax.loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=etwork_gmap_api_post_load&key=${encodeURIComponent(key)}`);
            });
        }
