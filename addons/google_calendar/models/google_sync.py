...
from etwork import api, fields, models, registry, _
from etwork.tools import ormcache_context
from etwork.exceptions import UserError
from etwork.osv import expression

from etwork.addons.google_calendar.utils.google_event import GoogleEvent
from etwork.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from etwork.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the etwork database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in etwork.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on etwork DB (_sync_google2etwork).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_etwork2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2etwork(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in etwork. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in etwork
        :return: synchronized etwork recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        etwork_values = [
            dict(self._etwork_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_etwork = self.with_context(dont_notify=True)._create_from_google(new, etwork_values)
        ...
        if not self._context.get("dont_notify") and all(not e.is_recurrence() for e in google_events):
            new_etwork._notify_attendees()

        cancelled = existing.cancelled()
        cancelled_etwork = self.browse(cancelled.etwork_ids(self.env))
        cancelled_etwork._cancel()
        synced_records = (new_etwork + cancelled_etwork).with_context(dont_notify=self._context.get("dont_notify", False))
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and etwork server time are different
            updated = parse(gevent.updated)
            etwork_record = self.browse(gevent.etwork_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not etwork_record.write_date or updated >= pytz.utc.localize(etwork_record.write_date):
                vals = dict(self._etwork_values(gevent, default_reminders), need_sync=False)
                etwork_record._write_from_google(gevent, vals)
                synced_records |= etwork_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from etwork to Google
    ...
    @api.model
    def _etwork_values(self, google_event: GoogleEvent, default_reminders=()):
    ...
    def _notify_attendees(self):
        ...
        This is called when creating new calendar events in _sync_google2etwork.
        At the initialization of a synced calendar, etwork requests all events for a specific
