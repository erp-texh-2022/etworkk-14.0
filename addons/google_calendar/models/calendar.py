...
class Meeting(models.Model):
    ...
    @api.model
    def _etwork_values(self, google_event, default_reminders=()):
        ...
        alarm_commands = self._etwork_reminders_commands(reminder_command)
        attendee_commands, partner_commands = self._etwork_attendee_commands(google_event)
        values = {
    ...
    @api.model
    def _etwork_attendee_commands(self, google_event):
        attendee_commands = []
        partner_commands = []
        ...
        if google_event.exists(self.env):
            existing_attendees = self.browse(google_event.etwork_id(self.env)).attendee_ids
        ...
        for etwork_attendee in attendees_by_emails.values():
            ...
            if tools.email_normalize(etwork_attendee.email) not in emails:
                attendee_commands += [(2, etwork_attendee.id)]
                partner_commands += [(3, etwork_attendee.partner_id.id)]
        return attendee_commands, partner_commands

    @api.model
    def _etwork_reminders_commands(self, reminders=()):
        commands = []
    ...
    def _google_values(self):
        ...
        values = {
            ...
            'extendedProperties': {
                'shared': {
                    '%s_etwork_id' % self.env.cr.dbname: self.id,
                },
            },
            'reminders': {
                'overrides': reminders,
        ...
        if self.user_id and self.user_id != self.env.user:
            ...
            # If several etwork users are attendees but the owner is not in etwork, the event will be duplicated on etwork database
            ...
            # values['extendedProperties']['private] should be used if the owner is not an etwork user
