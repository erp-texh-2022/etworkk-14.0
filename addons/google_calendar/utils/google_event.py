...
class GoogleEvent(abc.Set):
    ...
    def etwork_id(self, env):
        self.etwork_ids(env)  # load ids
    ...
    def _meta_etwork_id(self, dbname):
        ...
        o_id = properties.get('%s_etwork_id' % dbname)
        if o_id:
            return int(o_id)

    def etwork_ids(self, env):
        ids = tuple(e._etwork_id for e in self if e._etwork_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_etwork_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_etwork_ids_from_metadata(env, model)

        return tuple(e._etwork_id for e in self)

    def _load_etwork_ids_from_metadata(self, env, model):
        unsure_etwork_ids = tuple(e._meta_etwork_id(env.cr.dbname) for e in self)
        ...
        for e in self:
            etwork_id = e._meta_etwork_id(env.cr.dbname)
            if etwork_id in o_ids:
                e._events[e.id]['_etwork_id'] = etwork_id

    def _load_etwork_ids_from_db(self, env, model):
        etwork_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in etwork_events}  # {google_id: etwork_id}
        existing_google_ids = etwork_events.mapped('google_id')
        for e in self:
            etwork_id = mapping.get(e.id)
            if etwork_id:
                e._events[e.id]['_etwork_id'] = etwork_id
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_etwork_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.etwork_ids(env)
        events.etwork_ids(env)

        return self.filter(lambda e: e._etwork_id)
