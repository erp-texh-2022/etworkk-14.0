...
from etwork.tests.common import new_test_user
...
class TestSyncGoogle2etwork(TestSyncGoogle):
    ...
    def sync(self, events):
        ...
        self.env['calendar.recurrence']._sync_google2etwork(google_recurrence)
        self.env['calendar.event']._sync_google2etwork(events - google_recurrence)
    ...
    def test_new_google_event(self):
        values = {
            ...
            'organizer': {'email': 'etworkcalendarref@gmail.com', 'self': True},
        ...
        self.env['calendar.event']._sync_google2etwork(GoogleEvent([values]))
    ...
    def test_attendee_cancelled(self):
        ...
        self.assertGoogleEventPatched(event.google_id, {
            ...
            'attendees': [{'email': 'etworkbot@example.com', 'responseStatus': 'declined'}],
            'extendedProperties': {'private': {'%s_etwork_id' % self.env.cr.dbname: event.id}},
    ...
    def test_recurrence(self):
        ...
        self.env['calendar.recurrence']._sync_google2etwork(GoogleEvent([values]))
    ...
    def test_simple_event_into_recurrency(self):
        ...
        recurrence = self.env['calendar.recurrence']._sync_google2etwork(GoogleEvent([values]))
        events = recurrence.calendar_event_ids.sorted('start')
        self.assertEqual(len(events), 3, "it should have created a recurrence with 3 events")
        event = self.env['calendar.event'].search([('google_id', '=', values.get('id'))])
