...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/etwork" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/etwork/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/etwork/* /home/pi/etwork/
rm -r /home/pi/etwork/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service etwork restart) &
