...
copy_raspios () {
    ...
    wget -c 'https://nightly.etwork.com/master/iotbox/iotbox-latest.zip' -O raspios/iotbox-latest.zip
    wget -c 'https://nightly.etwork.com/master/iotbox/SHA1SUMS.txt' -O raspios/SHA1SUMS.txt
    cd raspios/
...
copy_iot () {
    ...
    # Modify startup
    mkdir -v etwork
    mount -v "${PART_IOTBOX_ROOT}" etwork
    cp -v /home/pi/upgrade.sh etwork/home/pi/
    NBR_LIGNE=$(sed -n -e '$=' etwork/etc/rc.local)
    sed -ie "${NBR_LIGNE}"'i\. /home/pi/upgrade.sh; clean_local' etwork/etc/rc.local
    find /home/pi/config -maxdepth 1 -type f ! -name ".*" -exec cp {} etwork/home/pi/ \;
