etwork.define('partner.autocomplete.Mixin', function (require) {
'use strict';
                ...
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    ...
                    clearbitSuggestions.forEach(function (suggestion) {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            etworkSuggestions.push(suggestion);
                        }
                    });
                }

                etworkSuggestions = _.filter(etworkSuggestions, function (suggestion) {
                    return !suggestion.ignored;
                });
                _.each(etworkSuggestions, function(suggestion){
                delete suggestion.ignored;
                });
                return resolve(etworkSuggestions);
            };

            self._whenAll([etworkPromise, clearbitPromise]).then(concatResults, concatResults);
        });
